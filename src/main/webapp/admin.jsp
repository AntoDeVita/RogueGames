...
<%@ include file="./fragments/header.jsp" %>
			    ...
			    <%
      			...
      			%>
			        <tr>
			            ...
			            <td><button class="adminBtn" onclick="showPopupModifica('<%=bean.getIdProdotti() %>','<%=bean.getNome() %>','<%=bean.getImmagine() %>','<%=bean.getVideo() %>', '<%=bean.getDescrizione() %>','<%=bean.getCoV() %>','<%=bean.getPrezzo() %>','<%=bean.getCasaProduttrice() %>','<%=bean.getPiattaforma() %>','<%=bean.getGenere() %>','<%=bean.getTipo() %>','<%=bean.getDataRilascio() %>','<%=bean.getQuantita() %>')">Modifica</button>
		...
		<div id="popupModifica">
			<form action="<%= request.getContextPath() %>/adminOperationServlet" method="POST"><%--Bottone Conferma Modifica act= 3 --%>
		        ...
		        <p>Immagine: <input type="file" id="img" name="img" accept=".png .jpg .jpeg"></input></p>
